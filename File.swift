...
class TheButton: UIButton {
    ...
    private var timerState = TimerState() // why is this happening when it's not happening to the other vars
    private var activeTimer: Timer? // why doesn't this need to be in the init is it the optional? 
    
    required init(timerView: TimerView, timerState: TimerState) {
